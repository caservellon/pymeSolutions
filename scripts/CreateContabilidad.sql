...
CREATE TABLE IF NOT EXISTS `pymeERP`.`CON_Proveedores` (
  ...
  `CON_MotivoTransaccion_ID` INT NOT NULL,
  `CON_Proveedores_FechaCreacion` DATETIME NULL DEFAULT NULL,
  `CON_Proveedores_FechaModificacion` DATETIME NULL DEFAULT NULL,
  `CON_Proveedores_UsuarioCreacion` VARCHAR(64) NULL,
  `CON_Proveedores_UsuarioModificacion` VARCHAR(64) NULL,
  PRIMARY KEY (`INV_Proveedores_ID`),
  UNIQUE INDEX `INV_Proveedores_ID_UNIQUE` (`INV_Proveedores_ID` ASC),
  INDEX `fk_CON_Proveedores_CON_MotivoTransaccion1_idx` (`CON_MotivoTransaccion_ID` ASC),
  CONSTRAINT `fk_CON_Proveedores_CON_MotivoTransaccion1`
    FOREIGN KEY (`CON_MotivoTransaccion_ID`)
    REFERENCES `pymeERP`.`CON_MotivoTransaccion` (`CON_MotivoTransaccion_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
