...
<p>{{ link_to_route('Inventario.Ciudad.index', 'Regresar a Ciudades') }}</p>
	...
	<tbody>
		<tr>
			<td>{{{ $Ciudad->INV_Ciudad_ID }}}</td>
                    ...
                    <td>{{ link_to_route('Inventario.Ciudad.edit', 'Edit', array($Ciudad->INV_Ciudad_ID), array('class' => 'btn btn-info')) }}</td>
                    <td>
